...
|---|---|---|
| **Un compte qui n'administre pas reçoit 404**, jamais 403 | les routes sont `authenticated` et la garde répond `notFound()`. **La raison écrite ici jusqu'à s56 est morte** : `RouteProtection.level: 'role'` rendait alors 403, il rend 404 depuis (ADR 068), donc ce n'est plus ce qui décide. Ce qui décide est la ligne « une session empruntée n'administre jamais » plus bas : `asSuperadmin` refuse l'emprunt **avant** de juger le rôle, et un niveau déclaré ne sait pas exprimer cet ordre — le répartiteur juge le rôle en premier, ou pas du tout. Basculer ces routes sur `role` perdrait cette garde | `tests/admin.test.ts` — le balayage de **toutes** les routes déclarées, dérivé du contrat |
