...
describe('le dernier superadmin', () => {
  ...
  it('refuse qu’on le bannisse', () => {
    expect(banRefusal({ superadminCount: 1, targetIsSuperadmin: true })).toBe('last_superadmin')
  })

  it('refuse qu’on le révoque', () => {
    expect(revocationRefusal({ superadminCount: 1, targetIsSuperadmin: true })).toBe(
      'last_superadmin',
    )
  })
  ...
  it('laisse bannir et révoquer dès qu’il en reste un autre', () => {
    ...
    expect(banRefusal({ superadminCount: 2, targetIsSuperadmin: true })).toBeNull()
    expect(revocationRefusal({ superadminCount: 2, targetIsSuperadmin: true })).toBeNull()
  })
  ...
  it('ne concerne pas un compte qui ne porte pas le rôle', () => {
    ...
    expect(banRefusal({ superadminCount: 1, targetIsSuperadmin: false })).toBeNull()
    // … mais lui **révoquer** un rôle qu'elle n'a pas est un refus distinct.
    expect(revocationRefusal({ superadminCount: 2, targetIsSuperadmin: false })).toBe(
      'not_superadmin',
    )
  })
})
