 ...
 *
 * Deux fonctions, comme `MarketingIntl` (s10), et pour la même raison : le
 * module ne connaît ni `next-intl`, ni le cookie de langue, ni le préfixe de
 * locale. Sa présentation se rend donc dans un test sans démarrer quoi que ce
 * soit, et elle ne porte aucune branche « si l'i18n existe » — `path` est
 * l'identité quand le module `i18n` est coupé.
 */
export interface BlogIntl {
