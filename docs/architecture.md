...
- **ADR 006 n'est enforcé qu'à moitié** : la règle de dépendance entre couches l'est, la **pureté du `domain`** (aucun framework, aucun ORM, aucun SDK) ne l'est pas encore. Le mécanisme existe (`boundaries/dependencies` avec un sélecteur `dependency` ; `boundaries/external` est déprécié) ; la liste de refus est tranchée en s03, avec le premier module réel.
