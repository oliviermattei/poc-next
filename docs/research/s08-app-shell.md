...
5. **Deux briques attendent déjà d'être consommées.** s03 a livré `visibleNavigation(registry, session)` — la navigation se construit depuis les modules activés, filtrée par la protection déclarée. s07 a livré les cas d'usage de compte : liste des sessions, révocation, changement de mot de passe et d'email avec révocation des autres sessions. **s08 les branche, il ne les réécrit pas.**
