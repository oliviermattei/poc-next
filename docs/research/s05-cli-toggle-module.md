...
4. **Préserver le formatage exige un AST, pas une expression régulière.** Le fichier porte des commentaires explicatifs destinés au propriétaire du projet, et une réécriture par expression régulière les détruirait au premier cas non prévu. `ts-morph` (28.0.0) donne cet AST. *Corrigé à l'exécution : ses API de manipulation (`addElement`, `insertElement`, `removeElement`) ne conservent pas le reste du fichier intact — elles reformatent la liste selon leurs propres réglages, emportent la virgule finale avec la dernière entrée et détruisent le commentaire de l'entrée suivante. L'AST ne sert donc qu'à **lire** des positions ; l'écriture est un découpage du texte d'origine (`packages/cli/src/features-file.ts`).*
